...
from utils.dataset import Flare7kpp_Pair_Loader
...
def collate_fn(examples):
    ...
    masked_images = _collate_imgs([example["masked_images"] for example in examples])
    conditioning_pixel_values = _collate_imgs(
...
def main(args):
    ...
    def tokenize_prompt(prompt, is_train=True):
        inputs = tokenizer(
            prompt,
    ...
    blip2.to(accelerator.device)

    # encoder_hidden_states = text_encoder(
    #     tokenize_prompt(args.instance_prompt, is_train=True).to(text_encoder.device)
    # )[0]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                ## generate specific prompt for each image using blip2 ##
                PIL_imgs = [
                    torchvision.transforms.ToPILImage()((img + 1.0) / 2.0)
                    for img in batch["masked_images"]
                ]
                inputs = processor(PIL_imgs, return_tensors="pt").to(
                    text_encoder.device, dtype=weight_dtype
                )
                ...
                # Append instance prompt to each generated text
                generated_text = [
                ...
                _encoder_hidden_states = text_encoder(text_prompts)[0]
