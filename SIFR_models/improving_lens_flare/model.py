...
class InputProj(tf.keras.Model):
    def __init__(self):
        ...
        self.proj = tf.keras.layers.Conv2D(
            32, 3, 1, activation="LeakyReLU", padding="same", input_shape=[512, 512, 3]
        )
...
def window_partition(x, win_size):
    ...
    windows = tf.reshape(
        tf.transpose(x, (0, 1, 3, 2, 4, 5)), (-1, win_size, win_size, C)
    )
    return windows
...
class LinearProjection(tf.keras.Model):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.0, bias=True):
        ...
        self.heads = heads
        self.to_q = tf.keras.layers.Dense(
            inner_dim, use_bias=bias, activation="LeakyReLU"
        )
        self.to_kv = tf.keras.layers.Dense(
            inner_dim * 2, use_bias=bias, activation="LeakyReLU"
        )
        self.dim = dim
    ...
    def call(self, x, attn_kv=None):
        ...
        q = tf.transpose(
            tf.reshape(self.to_q(x), (-1, N, 1, self.heads, C // self.heads)),
            (2, 0, 3, 1, 4),
        )
        kv = tf.transpose(
            tf.reshape(self.to_kv(attn_kv), (-1, N_kv, 2, self.heads, C // self.heads)),
            (2, 0, 3, 1, 4),
        )
        q = q[0]
        k, v = kv[0], kv[1]
    ...
    def __init__(
        ...
        self.relative_position_bias_table = tf.Variable(
            tf.random.truncated_normal(
                ((2 * win_size[0] - 1) * (2 * win_size[1] - 1), num_heads), stddev=0.02
            )
        )
        ...
        relative_position_bias = tf.reshape(
            tf.gather(
                ...
                tf.reshape(
                    ...
                    (tf.size(self.relative_position_index), 1),
                ),
            ),
            (
...
class LeFF(tf.keras.Model):
    def __init__(
        ...
        self.dwconv = tf.keras.layers.Conv2D(
            hidden_dim, groups=hidden_dim, kernel_size=3, strides=1, padding="same"
        )
        self.g2 = act_layer
...
class LeWinTransformerBlock(tf.keras.Model):
    ...
    def call(self, x):
        ...
        x_windows = window_partition(
            x, self.win_size
        )  # return windows with size             (N, win, win, C)
        x_windows = tf.reshape(
            x_windows, (-1, self.win_size * self.win_size, C)
        )  # reshape to (N, win* win, C)
        wmsa_in = x_windows
...
class Downsample(tf.keras.Model):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv = tf.keras.layers.Conv2D(
            out_channel, kernel_size=4, strides=2, padding="same"
        )
...
class Upsample(tf.keras.Model):
    def __init__(self, in_channel, out_channel):
        ...
        self.deconv = tf.keras.layers.Conv2DTranspose(
            out_channel, kernel_size=2, strides=2
        )
        self.in_channel = in_channel
        self.out_channel = out_channel
...
class Uformer(tf.keras.Model):
    def __init__(
        ...
        self.mlp_ratio = mlp_ratio
        enc_dpr = [
            x.item() for x in np.linspace(0, 0.1, sum(depths[: self.num_enc_layers]))
        ]
        conv_dpr = [drop_path_rate] * depths[4]
        dec_dpr = enc_dpr[::-1]
        self.input_proj = InputProj()
        self.output_proj = OutputProj(
            in_channel=2 * embed_dim, out_channel=3, kernel_size=3, stride=1
        )
